...
#define HI_BIT  (1L << (2 * N - 1))
			...
			{
#if pdp11
					static float two16m; /* old pdp11 cc can't compile an expression */
					two16m = 1.0 / (1L << N); /* in "float" initializer! */
#else
					static float two16m = 1.0 / (1L << N);
#endif
			...
			float rint(float x)
			{
				float d_FractionOfX = x - floor(x);    //fraction part of x
