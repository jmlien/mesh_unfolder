				   ...
				   )
{
	...
	if ((curvex.size() > 0 && curvey.size() > 0 && curvex.size() == curvey.size()) == false)
		cout << "curvex size=" << curvex.size() << " curvey size=" << curvey.size() << endl;

	assert(curvex.size()>0 && curvey.size()>0 && curvex.size()==curvey.size());


	if (isOpen==false) //closed
	{
		if (curvex.front() != curvex.back())
		{
			assert(false);
		}

		if (curvey.front() != curvey.back())
		{
			assert(false);
		}
	}
